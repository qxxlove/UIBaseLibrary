 ...
 *      这三种集合类型分别有存在MutableList<E>、MutableSet<E>、MutableMap<K,V>接口，这些接口中提供了改变、操作集合的方法。
 *            例如add()、clear()、remove()等函数。
 *
 *    在定义集合类型变量的时候如果使用List<E>、Set<E>、Map<K,V>声明的时候该集合则是不可变集合，
 *    而使用MutableList<E>、MutableSet<E>、MutableMap<K,V>的时候该集合才是可变类型集合。
 *
 * 协变
 *   当一个集合赋值给另外一个集合时，这里以List<E>举例，
 *           如果两个集合的类型也就是E类型相同时，赋值是没有问题的。
...
class ListAndArrayActivity : AppCompatActivity() {
    ...
    // 确定元素的值类型
    val mutableList2 = mutableListOf<String>("1", "2", "3", "4", "5")
    // 可传入一个数组
    val mutableList3 = mutableListOf(arr)
    // 这里的ArrayList<>和Java里面的ArrayList一致
    lateinit var  mutableList: ArrayList<String>

    val set1 = setOf(1,2,"3","4","2",1,2,3,4,5)
    val mutableSet1 = mutableSetOf(1,2,"3","4","2",1,2,3,4,5)
    // 这里的HashSet<>和Java里面的HashSet<>一致
    lateinit var  mutableSet2 : HashSet<String>
